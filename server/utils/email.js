...
const emailToken = async (token, receiver, purpose) => {
  try {
    ...
    const output1 = `<p>Hello ${
      receiver.username
    }</p>Link for reset password: <a  href ="http://ec2-18-234-196-219.compute-1.amazonaws.com/resetpassword/${token}"> <button>Reset</button></p><p>${
    ...
    const output2 = `<p>Hello ${
      receiver.username
    }</p>Link for email verification:<a href ="http://ec2-18-234-196-219.compute-1.amazonaws.com/verifyemail/${token}"> <button>Verify</button></a> <p>${
      receiver.role != "client" ? `Your code is ${receiver.uniqueCode}` : ""
